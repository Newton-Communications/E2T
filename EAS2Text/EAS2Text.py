...
class EAS2Text(object):
    ...
    try:
        ...
        sameusfile = open(inp_file)
        # with inp_file.open("rb") as f:  # or "rt" as text file with universal newlines
        #     template = f.read()
    except AttributeError:
        # # Python < PY3.9, fall back to method deprecated in PY3.11.
        # template = impresources.read_text(templates, 'same-us.json')
        # or for a file-like stream:
        sameusfile = impresources.open_text(templates, 'same-us.json')

    try:
        inp_file2 = (impresources.files(templates) / 'wfo-us.json')
        wfousfile = open(inp_file2)
        # with inp_file.open("rb") as f:  # or "rt" as text file with universal newlines
        #     template = f.read()
    except AttributeError:
        # # Python < PY3.9, fall back to method deprecated in PY3.11.
    ...
    def __init__(
        ...
        stats = load(self.sameusfile)
        locality2 = load(self.wfousfile)
        self.FIPS = []
        self.FIPSText = []
        self.strFIPS = ""
